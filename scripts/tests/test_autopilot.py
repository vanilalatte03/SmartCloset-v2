...
def test_codex_review_prompt_excludes_issue_records_and_uses_step_contract(runner):
    ...
    (root / "docs" / "RECOMMENDATION_RULES.md").write_text("# Rules\n", encoding="utf-8")
    skill_path = root / ".agents" / "skills" / "smartcloset-backend" / "SKILL.md"
    skill_path.parent.mkdir(parents=True)
    skill_path.write_text("# SmartCloset Backend Skill\n", encoding="utf-8")
    (root / ".codex" / "project-profile.json").write_text(
        json.dumps(
            {
                "reviewDocs": ["docs/API.md", "docs/RECOMMENDATION_RULES.md"],
                "projectSkillFiles": [".agents/skills/smartcloset-backend/SKILL.md"],
            }
        ),
        encoding="utf-8",
    )
    ...
    assert "`docs/RECOMMENDATION_RULES.md`" in prompt
    assert "`.agents/skills/smartcloset-backend/SKILL.md`" in prompt
