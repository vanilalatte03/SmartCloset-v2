                    ...
                    "Stop": [
                        {
                            "hooks": [
                                {
                                    "type": "command",
                                    "command": "sh \"$(git rev-parse --show-toplevel)/.codex/hooks/tdd-guard.sh\" stop",
                                }
                            ]
                        }
                    ]
...
def test_instance_mode_detects_direct_python_hook_command(tmp_path, monkeypatch):
    _write_ready_files(tmp_path)
    monkeypatch.setattr(doctor, "_git_hooks_path", lambda root=tmp_path: ".githooks")
    (tmp_path / ".codex" / "hooks.json").write_text(
    ...
    assert any("tdd-guard.sh so Python launcher fallback is used" in issue for issue in issues)
    assert any("must not depend on one Python executable name" in issue for issue in issues)


def test_instance_mode_detects_cwd_sensitive_hook_command(tmp_path, monkeypatch):
    _write_ready_files(tmp_path)
    monkeypatch.setattr(doctor, "_git_hooks_path", lambda root=tmp_path: ".githooks")
    (tmp_path / ".codex" / "hooks.json").write_text(
        '{ "command": "sh .codex/hooks/tdd-guard.sh stop" }',
        encoding="utf-8",
    )

    issues = doctor.collect_issues(tmp_path, "instance")

    assert any("must resolve the repository root" in issue for issue in issues)
...
def test_instance_mode_detects_missing_project_skill_file(tmp_path, monkeypatch):
    _write_ready_files(tmp_path)
    monkeypatch.setattr(doctor, "_git_hooks_path", lambda root=tmp_path: ".githooks")
    (tmp_path / ".codex" / "project-profile.json").write_text(
        json.dumps(
            {
                ...
                "guardMode": "soft",
                "commands": {},
                "projectSkillFiles": [".agents/skills/demo/SKILL.md"],
            }
        ),
        encoding="utf-8",
    )
    ...
    assert "Project skill file is missing: .agents/skills/demo/SKILL.md" in issues
