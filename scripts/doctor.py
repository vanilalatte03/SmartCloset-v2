...
LEGACY_HOOK_MARKERS = ("/bin/bash", "/usr/bin/python3")
DIRECT_PYTHON_HOOK_MARKERS = (
    "python .codex/hooks/tdd-guard.py",
    "python3 .codex/hooks/tdd-guard.py",
    "py -3 .codex/hooks/tdd-guard.py",
)
...
def _profile_issues(root: Path, mode: str) -> list[str]:
    ...
    issues.extend(_project_skill_issues(root, profile))
    return issues
...
def _project_skill_files(profile: dict) -> list[str]:
    configured = profile.get("projectSkillFiles", [])
    if not isinstance(configured, list):
        return []
    return [item for item in configured if isinstance(item, str) and item]


def _project_skill_issues(root: Path, profile: dict) -> list[str]:
    configured = profile.get("projectSkillFiles", [])
    if configured in (None, []):
        return []
    if not isinstance(configured, list):
        return [".codex/project-profile.json projectSkillFiles must be a list of relative file paths."]

    issues: list[str] = []
    for rel in configured:
        if not isinstance(rel, str) or not rel:
            issues.append(".codex/project-profile.json projectSkillFiles entries must be non-empty strings.")
            continue
        path = Path(rel)
        if path.is_absolute() or ".." in path.parts:
            issues.append(f".codex/project-profile.json projectSkillFiles entry '{rel}' must be a safe relative path.")
            continue
        if not (root / path).is_file():
            issues.append(f"Project skill file is missing: {rel}")
    return issues
...
def _template_contract_issues(root: Path) -> list[str]:
    ...
    issues.extend(_hook_contract_issues(root))
...
def _hook_contract_issues(root: Path) -> list[str]:
    hooks_json = _read_text(root, ".codex/hooks.json")
    if not hooks_json:
        return []

    issues: list[str] = []
    if "tdd-guard.sh" not in hooks_json:
        issues.append(".codex/hooks.json must route through tdd-guard.sh so Python launcher fallback is used.")
    if "tdd-guard.sh" in hooks_json and "git rev-parse --show-toplevel" not in hooks_json:
        issues.append(".codex/hooks.json must resolve the repository root before invoking tdd-guard.sh.")
    if any(marker in hooks_json for marker in DIRECT_PYTHON_HOOK_MARKERS):
        issues.append(".codex/hooks.json must not depend on one Python executable name; use tdd-guard.sh.")
...
def main(argv: list[str] | None = None, root: Path = ROOT) -> int:
    ...
    project_skills = _project_skill_files(profile)
    if project_skills:
        print("\nProject skill files")
        for rel in project_skills:
            print(f"- {rel}: {_status((root / rel).is_file())}")
