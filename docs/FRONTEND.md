...
- 같은 위치의 짧은 사용 흐름에서는 서버의 weather provider가 현재 날씨와 추천 생성 날씨를 같은 snapshot으로 맞출 수 있다.
...
필수 구성:
- 선택된 현재 위치 카드
- 현재 위치 기준 현재 날씨 요약 카드
- keyword 검색
- 선택 가능한 검색 결과 카드 목록
- 위치 선택 CTA

현재 날씨 요약은 기존 보호 API `GET /api/weather/current`를 사용한다. 날씨 요약 조회에 실패해도 위치 검색과 위치 선택은 계속 사용할 수 있어야 한다.

위치 저장 성공 문구는 `부산광역시로 저장했습니다.`처럼 `로/으로` 조사를 확정해서 표시한다. `(으)로` placeholder를 화면에 그대로 노출하지 않는다.
...
- 착용 여부 표시
- 이력 항목의 착용 완료 처리
- 날짜, 착용 여부, 추천 옷 조합, 간단 weather badge를 먼저 보여주는 요약 카드
- 추천 이유, 추천 당시 weather snapshot, 점수 상세를 낮춘 펼침 영역
