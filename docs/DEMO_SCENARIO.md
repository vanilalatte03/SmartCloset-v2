...
## P0 Release Cut 기준
Step 7 P0 데모는 Today 화면에서 첫 추천을 성공시키는 흐름을 기준으로 한다. 전용 History view의 이력 카드 polish는 P1 범위이며, P0에서는 Today의 최근 추천 preview와 추천 결과 착용 완료 흐름을 확인한다.

Step 8-13은 선호도, 위치, 이력 화면 polish와 Today/Closet/Preferences/Location/History의 시각 우선순위 보강 tail이다. 이 단계들이 남아 있어도 공개/보호 API 계약, Today 첫 추천 흐름, Closet 빠른 관리, responsive app shell이 통과하면 P0 공유 후보로 본다.
...
- 위치 화면은 선택된 현재 위치 카드, 현재 날씨 요약 카드, 선택 가능한 catalog 검색 결과 카드로 구성된다.
- 현재 날씨 요약 조회에 실패해도 위치 검색과 선택은 계속 가능하다.
...
- 이력은 최신순이다.
- 이력 카드에서 날짜, 착용 여부, 추천 옷 조합, 간단 weather badge를 먼저 확인할 수 있다.
- 추천 이유, 추천 당시 날씨 snapshot, 점수 상세는 펼침 영역에서 확인한다.
...
- 전용 History view의 모바일 이력 카드 polish는 P1 tail에서 완료한다.
