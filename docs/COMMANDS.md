...
`docker-compose.prod.yml`은 local/demo `docker-compose.yml`과 별도 파일이다. `.env.prod.example`은 secret 값을 비워 둔 checklist이므로 그대로 기동하지 않고, 운영 환경 또는 smoke 전용 임시 env에서 `MYSQL_PASSWORD`, `MYSQL_ROOT_PASSWORD`, `SPRING_DATASOURCE_PASSWORD`, `JWT_SECRET`, `CORS_ALLOWED_ORIGINS`, `FRONTEND_AUTH_CALLBACK_URL`, `VITE_API_BASE_URL`을 채워야 한다. Prod compose는 `SPRING_PROFILES_ACTIVE=prod`, `SMARTCLOSET_SEED_ENABLED=false`, `SPRINGDOC_API_DOCS_ENABLED=false`, `SPRINGDOC_SWAGGER_UI_ENABLED=false`, refresh/OAuth state cookie `Secure=true`를 기준으로 한다. Frontend service는 `frontend/Dockerfile`로 Vite build 산출물을 만들고 Nginx non-root static server로 서빙한다.
