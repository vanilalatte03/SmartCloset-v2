...
현재는 Java 21 Spring Boot 4.0.6 + Gradle + Docker Compose + React/Vite/TypeScript frontend 기준입니다. MVP6는 MVP5 옷 이미지 업로드 완료 baseline 위에 추천 상황, 추천 피드백, 옷별 styleTags, 최근 피드백 기반 개인화를 추가합니다.
...
| autopilot | `python3 scripts/autopilot.py 6-smartcloset-feedback-personalization --base main --max-review-fixes 2 --unsafe` | no | MVP6 step별 PR 생성, 자체 리뷰, 이슈 기록, 자동 병합 루프 |
...
MVP6 최종 수동 QA에서는 브라우저에서 옷별 styleTags 저장, 상황 선택 추천, 착용 완료, 추천 피드백 저장/clear, 추천 이력의 상황/착용/피드백 표시, 기존 이미지 업로드/썸네일 유지 여부를 확인한다.
