...
`python3 scripts/doctor.py --instance`가 통과한 뒤, Plan Mode에서 Harness skill로
...
- base 브랜치에서 `python3 scripts/checks.py --stage manual`이 통과

사전 조건이 충족되면 아래 명령으로 phase 전체 구현을 진행해줘:

python3 scripts/autopilot.py {phase-name} --max-review-fixes 2
