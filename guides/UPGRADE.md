...
| `.agents/skills/harness/`, `.agents/skills/review/` 같은 공통 Harness 스킬 | 템플릿 | 해당 공통 스킬만 덮어쓰기 |
...
| `AGENTS.md`, `docs/`, `phases/`, `issues/`, `archive/`, 프로젝트 전용 `.agents/skills/*` | 프로젝트 | 덮어쓰지 않음 |
...
프로젝트 전용 스킬은 `.codex/project-profile.json`의 `projectSkillFiles`에 기록해
둡니다. 업그레이드 후 `python3 scripts/doctor.py --instance`가 이 목록의 파일이
남아 있는지 확인하므로, SmartCloset처럼 `.agents/skills/smartcloset-backend/SKILL.md`를
AGENTS에서 참조하는 프로젝트는 해당 파일을 템플릿 덮어쓰기 대상에 포함하지 않습니다.
...
5. `python3 -m pytest scripts/tests`와 `python3 scripts/doctor.py --instance`가 통과하는지
   확인합니다.
