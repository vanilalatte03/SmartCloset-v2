...
```json
{
  ...
  "reviewDocs": ["docs/API.md", "docs/RECOMMENDATION_RULES.md"],
  "projectSkillFiles": [".agents/skills/project-domain/SKILL.md"]
}
```

`guardrailDocs`는 `execute.py`가 step 구현 프롬프트에 고정으로 첨부할 문서입니다.
비어 있으면 phase/step 문서에서 참조한 `docs/*.md`만 첨부하고, 참조가 없을 때
전체 docs를 fallback으로 첨부합니다.

`reviewDocs`는 `autopilot.py`의 read-only review 프롬프트에 명시할 프로젝트 핵심
계약 문서입니다. API, 추천 규칙, 프론트 계약처럼 자동 리뷰가 반드시 확인해야 하는
문서를 둡니다.

`projectSkillFiles`는 템플릿이 아니라 프로젝트가 소유하는 skill 파일 목록입니다.
`doctor.py --instance`는 이 파일들이 업그레이드 후에도 남아 있는지 확인하고,
`autopilot.py`는 read-only review 프롬프트에 함께 명시합니다.
