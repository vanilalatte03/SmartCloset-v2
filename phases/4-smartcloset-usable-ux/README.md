...
- Should-have / MVP4 P1: 인증 화면 한국어 정리, 선호도 swatch/chip UI, 위치 catalog 검색/선택 polish, 추천 이력과 착용 완료 polish, 로딩/빈 상태/저장 성공/인증 만료 문구 정리
...
- Step 7까지가 MVP4 P0 release cut이다. Step 8, 9, 10은 P1 polish tail이며, P0 완료를 막는 blocker로 보지 않는다.
- Step 8, 9, 10은 각 화면 polish를 분리한다. 한 화면의 부족을 다른 화면 step에서 선행 구현해서 해결하지 않는다.
...
- Step 8, 9, 10은 P1 화면 polish 기준으로 리뷰한다.
