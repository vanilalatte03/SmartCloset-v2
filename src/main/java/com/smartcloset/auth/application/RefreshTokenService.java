...
public class RefreshTokenService {
    ...
    public RotatedRefreshToken rotate(String refreshToken) {
        ...
        RefreshSession currentSession = refreshSessionRepository.findByTokenHashForRotation(currentHash)
        ...
        String nextTokenHash = hash(nextRefreshToken);
        currentSession.replace(nextTokenHash, now);
        RefreshSession nextSession = RefreshSession.issue(
                currentSession.getUser(),
                nextTokenHash,
                now,
                now.plusDays(ttlDays)
        );
        refreshSessionRepository.save(nextSession);
        return new RotatedRefreshToken(currentSession.getUser(), nextRefreshToken);
    }
    ...
    public void revokeIfPresent(String refreshToken) {
        String tokenHash = hash(refreshToken);
        refreshSessionRepository.findByTokenHashForRotation(tokenHash)
